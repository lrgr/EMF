...
#         self.X_source = X_src
#         self.X_val = X_val
#         self.tgt_matched_idxs = tgt_matched_idxs
#         self.src_matched_idxs = src_matched_idxs
#         self.sim_scores = sim_scores
#         self.verbosity=verbosity
...
#         V_tgt = tf.gather(self._VT, self.tgt_matched_idxs)
#         V_src = tf.gather(self._VS, self.src_matched_idxs)

#         weighted_diff = tf.multiply(self.sim_scores[:,tf.newaxis], (V_tgt - V_src))
#         sim_sqr_err = tf.square(tf.norm(weighted_diff))
#         sim_sqr_err_reg = 0.5 * self.lambda_sim * sim_sqr_err
...
def restrict_sim_scores(sim_scores, tgt, src):
    df = sim_scores[sim_scores.iloc[:, 0].isin(tgt)]
    df = df[df.iloc[:, 1].isin(src)]
    return df

def index_sim_scores_by_ints(sim_scores, tgt, src):
    tgt_g2i = gene2index(tgt)
    src_g2i = gene2index(src)
    scores = np.asarray([(tgt_g2i[t], src_g2i[s], score) for t, s, score in sim_scores.values])
    return scores
...
def main():
    ...
    assert(all(sim_scores_data['rows'] == tgt_gis['cols']))
    assert(all(sim_scores_data['cols'] == src_gis['cols']))

    # sim_scores = pd.read_csv(args.sim_scores, header=0, sep='\t')
    # log.info('\t- %d scores', len(sim_scores))
    # log.info(sim_scores.head())

    # log.info('[restricting sim scores]')
    # sim_scores = restrict_sim_scores(sim_scores, tgt_gis['cols'], src_gis['cols'])
    # scores = index_sim_scores_by_ints(sim_scores, tgt_gis['cols'], src_gis['cols'])
    # scores = normalize_sim_scores(scores)
    ...
    src_scaler = MCScaler('std')
    tgt_scaler = MCScaler('std')
    X_src = src_scaler.fit_transform(src_gis['values'])

    X_train, X_test, eval_mask = gi_train_test_split(tgt_gis, 0.7)

    X_train = tgt_scaler.fit_transform(X_train)
    tgt_gis['values'] = X_train 

    X_train, x_val, _ = gi_train_test_split(tgt_gis, 0.1)

    xsmf2 = XSMF2(X_train, X_src,
                  sim_scores,
                  rank=50,
                  X_val=x_val,
                  lambda_u=30,
    ...
    fitted = tgt_scaler.inverse_transform(xsmf2.X_fitted)

    pred = fitted[eval_mask]
    true = X_test[eval_mask]

    results = evaluate_model(true, pred)
